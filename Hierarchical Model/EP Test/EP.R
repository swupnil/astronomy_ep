...
EP = function(fit = NULL, data = NULL, J = 360, K = 6, prior.Mu = log(c(200, 250, 5, .5, 50, 7, 1, 50, .5)), 
              prior.Sigma.inv = diag(9) * 6/10, S = 10, SMOOTH = 0.9, randomSites = FALSE, parallel = TRUE, mc_iter = 100){
  ...
  for(s in 1:S){
    ...
    for(k in 1:K){
      ...
      for(i in 1:4) { init.data[[i]] <- list(beta0 = rep(0, B), beta1 = rep(0, B), phi = Mu.cav);}
